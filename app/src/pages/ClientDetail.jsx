      ...
      {tasks.map(task => (
        <div key={task.id} className="flex items-start gap-2.5 group">
          <div
            ...
            className={`w-4 h-4 mt-0.5 flex-shrink-0 flex items-center justify-center cursor-pointer transition-colors duration-150 ${
              task.completed
          ...
          <span className={`text-sm leading-relaxed flex-1 ${task.completed ? 'text-muted-foreground line-through' : 'text-foreground group-hover:text-primary transition-colors duration-150'}`}>
            {task.description}
          </span>
          <button onClick={() => onDelete(task.id)} className="opacity-0 group-hover:opacity-100 text-xs text-destructive/60 hover:text-destructive transition-all duration-150 flex-shrink-0">✕</button>
        </div>
      ))}
    </div>
  )
...
export default function ClientDetail() {
  ...
  const consultationTasks = tasks.filter(t => t.source === 'consultation')
  const tasksBySession = tasks.reduce((acc, t) => {
    if (t.session_id) {
    ...
    <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-background border border-border w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border flex-shrink-0">
              <span className="font-mono text-xs tracking-widest uppercase text-muted-foreground">諮詢報告</span>
      ...
      <main className="max-w-4xl mx-auto px-8 py-10">
        ...
        <div className="space-y-10">
          ...
          {/* 學生背景 */}
          <section>
            <SectionTitle>學生背景</SectionTitle>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
              <Field label="背景狀況" value={client.situation} />
              <Field label="個人特質" value={client.personality} />
              <Field label="現有基礎" value={client.skills} />
            </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3 flex-1">
                <span className="font-mono text-xs tracking-widest uppercase text-muted-foreground">初次諮詢</span>
                <div className="flex-1 h-px bg-border" />
              </div>
              {consultation && (
                <button
              ...
              : (
                <div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
                    ...
                    <Field label="教材/工具" value={consultation.tools} />
                  </div>
                  <Field label="諮詢摘要" value={consultation.summary} />
                  {consultation.project_proposals?.length > 0 && (
                    <div className="mb-4">
                      <p className="font-mono text-xs tracking-wider uppercase text-muted-foreground mb-2">學習計畫</p>
          ...
          <section>
            <div className="flex items-center gap-3 mb-4">
              <span className="font-mono text-xs tracking-widest uppercase text-muted-foreground">課程進度</span>
              <div className="flex-1 h-px bg-border" />
            </div>

            {sessions.length === 0 && consultationTasks.length === 0
              ...
              : (
                <>
                  ...
                  <button
                    ...
                    className="w-full text-left border border-border px-5 py-4 flex items-center justify-between hover:border-foreground/30 transition-colors duration-200"
                  >
                    <div className="flex items-center gap-6">
                      <div>
                        <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-0.5">堂數</p>
                        <p className="font-mono text-sm text-foreground">{sessions.length} 堂</p>
                      </div>
                      <div>
                        <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-0.5">作業完成</p>
                      ...
                      {sessions.length > 0 && sessions[sessions.length - 1].date && (
                        <div className="hidden sm:block">
                          <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-0.5">最近上課</p>
                          <p className="font-mono text-sm text-foreground">{formatDate(sessions[sessions.length - 1].date)}</p>
                        </div>
                      )}
                    </div>
                    <span className="font-mono text-xs text-muted-foreground ml-4 flex-shrink-0">
                      {progressOpen ? '收起 ↑' : '展開 ↓'}
                    </span>
                  </button>
                  ...
                  {/* 展開詳情 */}
                  {progressOpen && (
                    <div className="border border-t-0 border-border px-5 pt-5 pb-2 space-y-0">
                      {consultationTasks.length > 0 && (
                        <div className="flex gap-5 pb-6">
                          <div className="flex flex-col items-center flex-shrink-0">
                            <div className="w-6 h-6 rounded-sm border border-border bg-card text-muted-foreground font-mono text-[10px] flex items-center justify-center">諮</div>
                            {sessions.length > 0 && <div className="w-px flex-1 bg-border mt-2" />}
                          </div>
                          <div className="flex-1">
                            <p className="font-mono text-xs text-muted-foreground mb-3">
                              {consultation?.date ? formatDate(consultation.date) : '初次諮詢'}
                            </p>
                            <TaskList tasks={consultationTasks} onToggle={toggleTask} onDelete={deleteTask} />
                          </div>
                        </div>
                      )}
                          ...
                          <div className="pb-6 flex-1">
                            {editingSessionId === s.id ? (
                              <div className="space-y-3">
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    ...
                                    <input type="number" value={editSessionForm.session_number}
                                  ...
                                  <div>
                                    ...
                                    <input type="datetime-local" value={editSessionForm.date}
                                ...
                                {['objectives', 'progress', 'notes'].map(field => (
                                  <div key={field}>
                                    ...
                                    <textarea rows={3} value={editSessionForm[field]}
                              ...
                              <>
                                ...
                                {tasksBySession[s.id]?.length > 0 && (
                                  <div className="mt-3">
                                    <p className="font-mono text-xs text-muted-foreground uppercase tracking-wider mb-2">本次任務指派</p>
                                    <TaskList tasks={tasksBySession[s.id]} onToggle={toggleTask} onDelete={deleteTask} />
                                  </div>
                                )}
                              </>
                            )}
